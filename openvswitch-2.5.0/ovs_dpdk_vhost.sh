...
DPDK_HOME="/root/work/ovs/dpdk-2.2.0"
echo $DPDK_HOME
#Load UIO Drivers
rmmod igb_uio
rmmod uio
...
$DPDK_HOME/tools/dpdk_nic_bind.py -u 01:00.0
$DPDK_HOME/tools/dpdk_nic_bind.py -u 01:00.1
$DPDK_HOME/tools/dpdk_nic_bind.py --bind=igb_uio 0000:01:00.0
$DPDK_HOME/tools/dpdk_nic_bind.py --bind=igb_uio 0000:01:00.1
...
#ovs-vswitchd --dpdk -c 2 -n 4 --socket-mem 512,0 -- unix:/usr/local/var/run/openvswitch/db.sock -vconsole:emer -vsyslog:err -vfile:info --mlockall --no-chdir --log-file=/usr/local/var/log/openvswitch/ovs-vswitchd.log --pidfile=/usr/local/var/run/openvswitch/ovs-vswitchd.pid --detach --monitor
#ovs-vswitchd --dpdk -c 0x2 -n 4 --socket-mem 1024 -- unix:/usr/local/var/run/openvswitch/db.sock --pidfile --detach

modprobe openvswitch
#set 400M for socket0 socket1 of dpdk
#ovs-vswitchd --dpdk -c 0x2 -n 4 --socket-mem 1024 -- unix:/usr/local/var/run/openvswitch/db.sock --pidfile --detach
ovs-vswitchd --dpdk -c 0x2 -n 4 --socket-mem 1024 -- unix:/usr/local/var/run/openvswitch/db.sock --log-file=/usr/local/var/log/openvswitch/ovs-vswitchd.log --pidfile --detach
